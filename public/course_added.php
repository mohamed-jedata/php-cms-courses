...
if(isset($_POST["submit"])  ){
    ...
    if(empty($errors)){
        ...
        $query = "INSERT INTO `courses`(`course_name`, `creator`, `path_id`,`description`,`intro_course`,`course_img`,`visible`) VALUES".
            "('$course_name','$creator','$path_id','$descripton','$course_intro','$course_img',$visible)" ;
        
        $result = mysqli_query($conn, $query);
        
        if($result && mysqli_affected_rows($conn) > 0){
            $_SESSION['msg'] = success_addCourse();
            redirect("manage_content.php");
        }else{
            $_SESSION['msg'] = failed_addCourse();
